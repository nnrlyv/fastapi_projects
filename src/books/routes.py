...
@book_router.get("/", response_model=List[BookSchema], dependencies=[role_checker]) #get all
async def get_all_books(session: AsyncSession = Depends(get_session),token_details:dict= Depends(access_token_bearer)):
    print(token_details)
    books = await book_service.get_all_books(session)
    return books

@book_router.post("/", status_code=status.HTTP_201_CREATED, response_model=BookSchema, dependencies=[role_checker]) #creating
...
@book_router.get("/{book_uid}", response_model=BookSchema, dependencies=[role_checker]) #get one book
...
@book_router.patch("/{book_uid}", response_model=BookSchema, dependencies=[role_checker]) #updating
...
@book_router.delete("/{book_uid}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[role_checker]) #deleting
